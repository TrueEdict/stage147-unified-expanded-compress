...
def stable_hash(text: str, dim: int, seed: int) -> np.ndarray:
    out=np.zeros(dim,np.float32); raw=text.encode('utf-8','replace')
    for n in (1,2,3,4):
...
def capture_teacher(model_id: str, rows: list[dict[str,str]], topk: int, seq_len: int, device: str, dtype_name: str) -> list[dict[str,float]]:
    import torch
    tok,model=load_teacher(model_id,device,dtype_name); actual_device=next(model.parameters()).device; captured=[]
    with torch.inference_mode():
        for row in rows:
            enc=tok(row['prompt'],return_tensors='pt',truncation=True,max_length=seq_len); enc={k:v.to(actual_device) for k,v in enc.items()}
            logits=model(**enc).logits[0,-1].float(); probs=torch.softmax(logits,dim=-1); vals,ids=torch.topk(probs,min(topk,probs.shape[-1])); dist={}
            for p,tid in zip(vals.cpu().tolist(),ids.cpu().tolist()):
                piece=canonical_piece(tok.decode([tid],clean_up_tokenization_spaces=False,skip_special_tokens=False)); dist[piece]=dist.get(piece,0.0)+float(p)
            z=sum(dist.values()) or 1.0; captured.append({k:v/z for k,v in dist.items()})
...
def main() -> int:
    p=argparse.ArgumentParser(); p.add_argument('--teacher-models',default='Qwen/Qwen3-0.6B,LiquidAI/LFM2-350M'); p.add_argument('--teacher-weights',default='0.5,0.5'); p.add_argument('--work-dir',default='/kaggle/working/stage147_real_dual_teacher'); p.add_argument('--device',default='auto'); p.add_argument('--dtype',choices=['float16','bfloat16','float32'],default='float16'); p.add_argument('--seed',type=int,default=147); p.add_argument('--repeats',type=int,default=8); p.add_argument('--shard-size',type=int,default=8); p.add_argument('--teacher-topk',type=int,default=64); p.add_argument('--seq-len',type=int,default=128); p.add_argument('--feature-dim',type=int,default=256); p.add_argument('--temp-hidden',type=int,default=1024); p.add_argument('--compress-rank',type=int,default=64); p.add_argument('--ridge',type=float,default=1e-2); p.add_argument('--max-shared-vocab',type=int,default=4096); p.add_argument('--resume',action='store_true')
    ...
    sw=sum(weights); weights=[w/sw for w in weights]; rows=build_examples(a.repeats,a.seed); manifest={'stage':147,'created_at_utc':utc(),'teachers':teachers,'teacher_weights':weights,'shard_size':a.shard_size,'total_examples':len(rows),'shards':[]}; all_dists=[]; all_rows=[]
    for shard_id,chunk in shard_rows(rows,a.shard_size):
        shard_file=shards_dir/f'shard_{shard_id:04d}.json'
        if a.resume and shard_file.exists(): payload=json.loads(shard_file.read_text()); merged=payload['merged_targets']
        else:
            captures=[capture_teacher(teacher,chunk,a.teacher_topk,a.seq_len,a.device,a.dtype) for teacher in teachers]; merged=merge_distributions(captures,weights); payload={'shard_id':shard_id,'created_at_utc':utc(),'teachers':teachers,'teacher_weights':weights,'rows':chunk,'merged_targets':merged}; dump(shard_file,payload)
        manifest['shards'].append({'shard_id':shard_id,'path':str(shard_file.relative_to(out)),'sha256':sha256_file(shard_file),'examples':len(chunk)}); all_dists.extend(merged); all_rows.extend(chunk); dump(out/'manifest.json',manifest)
    mass={}
    for d in all_dists:
        for t,v in d.items(): mass[t]=mass.get(t,0.0)+v
    vocab=[t for t,_ in sorted(mass.items(),key=lambda kv:(-kv[1],kv[0]))[:a.max_shared_vocab]]; X=np.stack([stable_hash(r['domain']+'\n'+r['prompt'],a.feature_dim,a.seed) for r in all_rows]); Y=vectorize_targets(all_dists,vocab)
    rng=np.random.default_rng(a.seed); A=rng.normal(0,1/math.sqrt(a.feature_dim),(a.feature_dim,a.temp_hidden)).astype(np.float32); H=X@A; Wtemp=np.linalg.solve(H.T@H+a.ridge*np.eye(a.temp_hidden,dtype=np.float32),H.T@Y); W=A@Wtemp; U,S,Vt=np.linalg.svd(W,full_matrices=False); rank=min(a.compress_rank,len(S)); Wc=(U[:,:rank]*S[:rank])@Vt[:rank]
    metrics=evaluate(Y,X@Wc); metrics.update({'examples':len(all_rows),'shared_vocab_size':len(vocab),'feature_dim':a.feature_dim,'temporary_hidden':a.temp_hidden,'compress_rank':rank,'temporary_parameters':int(a.feature_dim*a.temp_hidden+a.temp_hidden*len(vocab)),'final_parameters':int(a.feature_dim*rank+rank*len(vocab))})
    ...
    report={'stage':147,'name':'real_dual_teacher_sharded_distillation','runtime_verdict':'PASS' if all(gates.values()) else 'FAIL','teachers':teachers,'teacher_weights':weights,'metrics':metrics,'gates':gates,'manifest_sha256':sha256_file(out/'manifest.json'),'checkpoint_sha256':sha256_file(ckpt_dir/'student_compressed.npz')}; dump(out/'stage147_real_report.json',report); (out/'FINAL_VERDICT.md').write_text(f"# Stage147 Real Dual-Teacher Sharded Distillation\n\n**{report['runtime_verdict']}**\n\nTeachers: {', '.join(teachers)}\n\nShards: {len(manifest['shards'])}\n",encoding='utf-8'); print(json.dumps(report,indent=2)); return 0 if report['runtime_verdict']=='PASS' else 1
